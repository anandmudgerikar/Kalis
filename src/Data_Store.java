...
public class Data_Store
{
	 ...
	 public void update_queues() throws IOException {
		...
		//we assume number of nodes is n=10
		 FileInputStream configstream = new FileInputStream("/home/odroid/tinyos-main/project/ids/data/Rawpacket_capture.txt");
...
//				 
				 ...
				 queues[Integer.parseInt(node_id)].add(message); //added to appropriate queue in data store
				 
			 }
			 else if ((strLine.substring(0,5)).equals("00 FE")) //condition for CPT check) (currently manually setting value to FE in packet) //errors in decoding.. needs fixing
			 {
